...
// function MainAnalyseShape({ selectedImage, imageUrl, handleImageClosing , onShapeDataChange , shapeDataFromChild }) {
...
//         if (shapes.length > 0) {
//             alert("Only one shape is allowed. Reset to create a new shape.");
//             return;
//         }
...
//     };
//     useEffect(() => {
//         console.log("Received shape data in MainAnalyseShape:", shapeDataFromChild);
//     }, [shapeDataFromChild]);
...
//         if (mode === 'rectangle' && shapes.length === 0) {
//             setRectProps({ x: adjustedPos.x, y: adjustedPos.y, width: 0, height: 0 });
//             setIsDrawing(true);
//         } else if (mode === 'polygon' && !isPolygonComplete && shapes.length === 0) {
...
//                 shape:rectProps
//             })
//             setShapes([{ type: 'rectangle', ...rectProps }]);
//             setIsDrawing(false);
//             setRectProps(null);
//             setMode(null);  // Reset mode to enable dragging after drawing
//         }
//     };
...
//                 shape:polygonPoints
//             })
//             setShapes([{ type: 'polygon', points: polygonPoints }]);
...
//         if (mode === 'polygon' && polygonPoints.length >= 3 && !isPolygonComplete) {
...
//         setIsPolygonComplete(false);
//         setRectProps(null);
//         setMode(null);  // Reset mode to enable dragging after reset
//     };
...
//                 <button onClick={() => handleModeChange('rectangle')} className="mx-2 p-2 bg-blue-500 text-white rounded">
//                     Create Rectangle
//                 </button>
//                 <button onClick={() => handleModeChange('polygon')} className="mx-2 p-2 bg-green-500 text-white rounded">
//                     Create Polygon
//                 </button>
//                 <button onClick={handleReset} className="mx-2 p-2 bg-red-500 text-white rounded">
//                     Reset
//                 </button>
//             </div>
//             {selectedImage && (
...
//                                                 draggable
//                                                 onDragMove={(e) =>
//                                                     handlePointDrag(shapeIndex, pointIndex / 2, e.target.x(), e.target.y())
//                                                 }
//                                             />
//                                         ) : null
//                                     )}
...
//         useEffect(() => {
//             // console.log("Received shape data in MainAnalyseShape:", shapeDataFromChild);
//             // Update shapes state based on shapeDataFromChild
//             if (shapeDataFromChild) {
//                 const updatedShapes = [];

//                 // Add rectangles from shapeDataFromChild.rectangles
//                 if (shapeDataFromChild.rectangles && shapeDataFromChild.rectangles.length > 0) {
...
//                 // Add polygons from shapeDataFromChild.polygons
//                 if (shapeDataFromChild.polygons && shapeDataFromChild.polygons.length > 0) {
...
//         useEffect(() => {
//             // Reset image position and scale when a new image is selected
//             if (selectedImage && stageRef.current) {
//                 stageRef.current.position({ x: 0, y: 0 });
//                 stageRef.current.scale({ x: 1, y: 1 });
//                 setScale(1); // Reset scale state as well
//             }
//         }, [selectedImage]);
...
//         };
//         useEffect(() => {
//             // Send updated shape data to parent whenever shapeData changes
//             if (onShapeDataChange && shapeData) {
//                 onShapeDataChange(shapeData);
//                 console.log("Shape data sent to parent:", shapeData); // Log for debugging
//             }
//         }, [shapeData, onShapeDataChange]);
//         const handleMouseMove = () => {
//             if (isDrawing && rectProps) {
...
//       <div
//         className="mx-2 p-2 border text-white rounded cursor-pointer"
//         data-tooltip-id="tooltip"
//         data-tooltip-content="Fast Inspection"
//       >
...
//                         overflow: 'hidden', // Ensures the Stage content is clipped
...
//                                 />
//                             )}
//                             {/* Render rectangles and polygons from shapes */}
...
//                                 )
//                             ))}
//                             {/* Show the shape being drawn, if applicable */}
//                             {isDrawing && rectProps && (
//                                 <Rect
//                                     x={rectProps.x}
//                                     y={rectProps.y}
...
//                 )}
//                 {selectedImage && (
//                     <p className="cursor-pointer text-white font-bold z-40 p-1   px-2 absolute top-[5px] right-[10px]" onClick={handleImageClosing}>
//                    <FontAwesomeIcon icon={faSquareXmark} beat/>
//                     </p>
//                 )}
//             </div>
//         );
...
function MainAnalyseShape({ selectedImage, imageUrl, handleImageClosing, onShapeDataChange, shapeDataFromChild , fastInspactionfromchild , sendfastmodal , sendFastInspactionDataToParent}) {
    ...
    useEffect(() => {
        // Load initial shapes if provided from parent
        
        console.warn(`setFastinspactionfromchildshape setFastinspactionfromchildshape ${fastInspactionfromchild}`)
